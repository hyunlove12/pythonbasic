...
def incr(a, step=1): # a를 step 만큼 증가시키는 함수
    # step의 기본값은 1, 호출 시 step을 명시하지 않으면 기본값 1을 사용
    return a + step


print("incr :", incr(10, 2))
print("incr(기본값 사용) :", incr(10)) # 두 번째 인수는 명시하지 않음 -> 기본값 사용
...
# 인수명 앞에 *을 붙여서 선언
def get_total(*nums):
    # 만약 nums 내부의 요소가 int, float형이면 합산
    # 그 이외의 자료형이면 합산하지 않음
    # print("가변인수 :", nums)
    total = 0
    for num in nums:
...
print(get_total(2, 4, 6, 8, 10))
print(get_total(2, 4, 6, '123', 8, 10)) # 123은 합산되지 않는다.
...
def calc(a, b, func):
    # func는 외부에서 함수를 전달
    if callable(func): # 전달받은 func가 함수인가?
        return func(a, b)


print("calc + add :", calc(10, 20, add)) # 함수의 이름만 명시해준다
print("calc + add :", calc(10, 20, substract))


# 예제 :
# 정제되지 않은 문자열이 있다
words = "python pRogrammIng, jaVa pRoGRAMMing, LINUX, WinDoWs".split(",") # ,로 분리
print("DIRTY String :", words)
...
def clean_string(strings, *funcs):
    ...
    for string in strings:
        # 함수의 목록을 루프
        for func in funcs:
            if callable(func): # 가변 인수로 넘어온 func가 함수면
                string = func(string)
        result.append(string)
    return result


print("CLEANED :", clean_string(words, str.strip, str.title)) # __ __무슨 의미? str.strip이 함수의미?
...
for i in range(1, 11): # 1 ~ 10까지 loop를 돌면서 square함수 수행
...
words = "Life is too short, you need Python".upper().replace(",", "")\
    .split()
print("words :", words)
