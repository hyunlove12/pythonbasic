...
def test_unbound_method():
    ...
    # 어떤 객체의 메서드인지 식별하기 위해 실제 인스터스를 전달
    p = Point()
    Point.set_x(p, 10)
    Point.set_y(p, 10)
...
def test_repr():
    ...
    # __repr__ 메서드로 출력된 문자열로 다시 객체를 복원할 수 있어야 한다
    p2 = eval(repr(p))
...
def test_oper_overloading():
    ...
    # class point 내에 __add__를 오버로딩 했다 -> 연산 수행

    print("int + Point :", 20 + Point(10, 10))
    # 기본적인 연산자 수행 실패했을 경우, 뒤쪽 클래스가
    # 다시 한번 점검할 수 있다 -> 역이행 연산자
    ...
    print("p1 :", p1)
    print("p2 :", p2)
    print("p1 == p2?", p1 == p2) # __eq__ 오버로딩 -> 객체 비교가 아닌 해당 값 비교로 수정
...
def extend_ex():
    ...
    # ColorPoint는 Point가 가진 모든 멤버와 메소드를 상속 받았다
    print("cp :", cp)
