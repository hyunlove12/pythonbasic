def define_set():
    ...
    # 순서가 없고 -> 인덱싱, 슬라이싱 불가
    # 중복이 없음
    # 길이를 측정 len, 포함 여부(in, not in) 확인할 수 있다.
    :return:
    """
    empty = set() # 빈 셋을 만들고자 할 경우는 {} 쓰면 안된다. -> dict
    print(empty, type(empty))

    # 빈 set의 경우 아래와 같이 선언하면 dict형으로 생성된다.
...
def set_methods():
    ...
    s1.add(4) # 셋에 객체 4 추가
    print("ADD :", s1)
    s1.add(4) # 이미 있는 객체 4 추가
    print("ADD 2nd :", s1) # 셋은 중복 허용하지 않음

    # 객체의 삭제
    # discard : 객체를 삭제, 없어도 에러 없음
    # remove : 객체를 삭제, 없으면 Error
    s1.discard(4) # 객체 4를 삭제
    print("DISCARD :", s1)
    s1.discard(4) # 없는 개체 4를 다시 삭제
    print("DISCARD :", s1) # discard는 Error발생 하지 않는다
...
def set_oper():
    ...
    print("numbers는 evens의 모집합?", numbers.issuperset(odds))
    ...
    # 차집합 -, difference
    print(numbers - evens == odds) # 전체집합에서 짝수 집합을 빼면 홀수
    print(numbers.difference(odds) == evens)

def loop():
    """
    셋의 반복
    # 순서가 없으므로 어떤 순서로 출력되는지는 알 수 없다
    :return:
    """
