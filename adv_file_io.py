# 파일 모드
# 파일의 타입에 대한 구분 : text(t - default), binary(b)
...
def write_test01():
    ...
    # 중요 : 파일을 open했으면 반드시 close
    f.close()


def read_test01():
    # 텍스트 파일을 읽어와서  출력
    f = open("./sample/test.txt", "rt", encoding="UTF-8")
    # 텍스트 파일 읽기 모드
    data = f.read()
    print("파일 내용 :", data)
    f.close()
...
def read_multiline():
    ...
    for index, line in enumerate(lines):
        print("{}번째 라인 : {}".format(index + 1, line))
...
def with_as():
    # 파일, DBMS, 네트워크 등 시스템 자원은 open 하고사용을 했으면
    # 반드시 닫아줘야 한다 -> with as
...
# Pickle 모듈
# 파이썬 객체 -> 바이트 스트림 형태로 직렬화
# 바이트 스트림 파일 -> 파이썬 객체로 역직렬화 해주는 모듈
import pickle # pickle 모듈을 사용하려면 import

def pickle_dump():
    # pickle은 바이트 스트림 형태로 직렬화 하므로 파일 모드는 반드시 binary모드
...
def pickle_load():
    ...
    with open("./sample/players.bin", "rb") as f:
        ...
        # load시에는 명시하지 않아도 된다. -> dump시 파일에 같이 저장되기 때문
        """
        print(pickle.load(f))
        print(pickle.load(f))
        print(pickle.load(f))
        """
        # 파일 내부에 직렬화 된 피클객체의 수보다
