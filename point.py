class Point:
    ...
    def __repr__(self):
        ...
        # 이 문자열을 활용, 객체를 복원가능해야 한다(개발자용)
        return "Point({}, {})".format(self.x, self.y)


    # 인스턴스 메서드
    #   첫번째 인자는 반드시 self이여야 한다
    def set_x(self, x):
        self.x = x
    ...
    def __add__(self, other): # + 연산자의 오버로딩
        # Point + Point ->
        # Point + int ->
        if isinstance(other, Point): # Point + Point 의 연산
    ...
    def class_method(cls): # cls는 해당 클래스의 참조
        # classmethod와 staticmethod구분에 대한 논의 -> 필요한가?
