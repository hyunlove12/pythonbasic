...
dbfile = "./database/mysqlite.db" # 데이터베이스 파일
...
def test_insert_data(name, category, region):
    ...
    conn.commit() # 데이터 변경을 실제 데이터베이스에 반영
    conn.close()
...
def insert_bulk_data():
    test_delete_all()
    test_insert_data("둘리", 1, "서초")
    test_insert_data("고길동", 2, "상계")
    test_insert_data("마이콜", 2, "상계")
    test_insert_data("홍길동", 3, "강남")
    test_insert_data("성춘향", 3, "강남")
...
def test_select_data():
    ...
    print("fetchmany") # 여러개 받아오기
    print(cursor.fetchmany(2)) # 커서의 위치에서 2개의 레코드
    print("fetchall") # 모든 레코드 -> 처리하고 남은 레코드
...
def test_search_data():
    ...
    SELECT * FROM customer
     WHERE region = ? 
    """
    cursor = conn.execute(sql, ("상계", )) # 튜플이지만 요소값이 1개이기 때문에 ,
